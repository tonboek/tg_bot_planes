...
#Самолёты
available = ['Ан-2']
all_planes =['Ан-12', 'Ту-104', 'Ту-114', 'Ил-18', 'Як-42', 'Ил-86', 'Ту-144', 'Ан-125', 'Ан-225', 'Бе-200', 'Ту-214Р', 'Sukhoi Superjet 100', 'МС-21']
...
@dp.message_handler(commands = ['my_hangar'])
async def send_hangar(message: types.Message):
    global available
    
    for i in available:
        if available.count(i) > 1:
            available.pop(available.index(i))
    ...
    await message.answer(f'<b>Список доступных самолётов:</b> \n{result1} \n<b>Самолёты, которые тебе ещё недоступны:</b> \n{result2}', parse_mode='HTML')
...
async def send_sticker(message: types.Message):
    await bot.send_sticker(message.from_user.id, sticker = 'CAACAgIAAxkBAAEIUVVkHvFuwTIWOAJ3xqjG5uPupAOPRwACTBcAAhvMGEm4tlYX5pz38C8E')
...
выпускают в Китае.'
    ...
    async def flight_cancel(start, stop, distance, sec):
        ...
        for i in range(1, distance):
            ...
            if flag == False:
                break
    txt = 'Великолепная посадка и отличная работа экипажа! Твой уровень повысился до <b>2</b>, а баланс равен <b>150 монетам</b>. Ты действительно заслужил этого, поэтому встречай в своём ангаре новый самолёт - <b>Ан-12</b>. \
...
разработан на хорошем техническом уровне. Самое то для тебя, поэтому предлагаю испытать его прямо сейчас!'
...
@dp.message_handler(text = ['Ту-104 - 50 монет', 'Ту-114 - 50 монет', 'Ил-18 - 50 монет'])
...
@dp.message_handler(text = ['Як-42 - 100 монет', 'Ил-86 - 100 монет', 'Ту-144 - 100 монет'])
...
рекордов, что является рекордом само по себе.'
        ...
        if message.text == 'Як-42 - 100 монет' or message.text == 'Ил-86 - 100 монет' or message.text == 'Ту-144 - 100 монет':  #кнопкаларны чыгарырга
            await message.answer('Твой баланс равен 500 монетам. Кого?', reply_markup=An_Transport)

@dp.message_handler(text = ['Ан-124 - 200 монет', 'Ан-225 - 200 монет'])
async def third_part(message: types.Message):
    global count
    ...
    if count == 2:
        ...
        if message.text == 'Ан-124 - 200 монет':
            ...
            await message.answer('Принято! Баланс: 300 монет.')
...
@dp.message_handler(text = ['Бе-200 - 250 монет', 'Ту-214Р - 250 монет'])
async def fourth_part(message: types.Message):
    global count
    if count == 3:
        count += 1
        async def gotovo(sec):
...
@dp.message_handler(text = ['Sukhoi Superjet 100 - 300 монет', 'MC-21 - 300 монет'])
async def fifth_part(message: types.Message):
    global count
    if count == 4:
        async def gotovo(sec):
            msg = await message.answer('Проверка документов')
        ...
        if message.text == 'Sukhoi Superjet 100 - 300 монет':
            ...
            await sleep(0.5)
            await message.answer('Баланс: 1000 монет. \nНу что ж, уважаемый представитель молодежи. Вот и подошло к концу наше с тобой путешествие. Я очень надеюсь, что \
тебе понравилось и было интересно. Думаю ты узнал для себя много чего нового, сильнее увлёкся авиацией. Удачи!')
